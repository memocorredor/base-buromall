...
class AdminUserNewController extends Controller
{
    ...
    public function __construct()
    {
        ...
        // SEO del form
        $this->seo_title = __('seo_admin.local_admin_user_news_title');
        $this->seo_description = __('seo_admin.local_admin_user_news_description');
        $this->seo_keywords = __('seo_admin.local_admin_user_news_keywords');
        $this->view_icon = 'fas fa-bullhorn';
        // Rutas del view and form
        $this->form_view = 'admin.user_news';
        $this->form_show = 'admin.user_news.show';
        $this->form_new = 'admin.user_news.new';
        $this->form_create = 'admin.user_news.store';
        $this->form_edit = 'admin.user_news.edit';
        $this->form_update = 'admin.user_news.update';
        $this->form_delete = 'admin.user_news.delete';
    }
    ...
    public function index()
    {
        ...
        // Render del view
        return view('web.admin.user_news.main', compact('data_item'), [
            'meta_lang' => $this->lang,
    ...
    public function show($id)
    {
        ...
        if ($data_item != null) {
            ...
            $this->user_sis = CoreUser::getUser();
            // Render del view
            return view('web.admin.user_news.form', [
                'meta_lang' => $this->lang,
                'seo_title' => $this->seo_title,
                'seo_description' => $this->seo_description,
                'meta_sis' => $for_meta_sis,
    ...
    public function store(Request $request)
    {
        if ($request->get('process') === 'add') {
            $this->validate($request, [
                ...
                'title_es' => 'required|min:10|max:60',
                'text_es' => 'required',
                'description_es' => 'required|min:10|max:155',
                'keywords_es' => 'required',
                'title_en' => 'required|min:10|max:60',
                'text_en' => 'required',
                'description_en' => 'required|min:10|max:155',
                'keywords_en' => 'required',
                'title_pt' => 'required|min:10|max:60',
                'text_pt' => 'required',
                'description_pt' => 'required|min:10|max:155',
                'keywords_pt' => 'required',
                'image.*' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048'
            ]);

            // Crea la instancia
            $data_field = new UserNew();
            ...
            $next_news_id = $data_field->id;
            //check if image exist
            if ($request->hasFile('image')) {
                $images = $request->file('image');
                //Ruta para guardar
                $url_save_or = 'upload/news/' . $next_news_id . '/';
