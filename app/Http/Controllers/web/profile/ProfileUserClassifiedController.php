...
use Buromall\Models\UserClassified;
...
class ProfileUserClassifiedController extends Controller
{
    ...
    public function __construct()
    {
        ...
        // SEO del form
        $this->seo_title = __('seo_profile.local_profile_user_classified_title');
        $this->seo_description = __('seo_profile.local_profile_user_classified_description');
        $this->seo_keywords = __('seo_profile.local_profile_user_classified_keywords');
        $this->view_icon = 'fas fa-bullhorn';
        // Rutas del view and form
        $this->form_view = 'profile.user_classified';
        $this->form_show = 'profile.user_classified.show';
        $this->form_new = 'profile.user_classified.new';
        $this->form_create = 'profile.user_classified.store';
        $this->form_edit = 'profile.user_classified.edit';
        $this->form_update = 'profile.user_classified.update';
        $this->form_delete = 'profile.user_classified.delete';
    }
    ...
    public function index()
    {
        ...
        // Render del view
        return view('web.profile.user_classified.main', compact('data_item'), [
            'meta_lang' => $this->lang,
    ...
    public function show($id)
    {
        ...
        if ($data_item != null) {
            ...
            $data_pictures = UserItemMedia::where('classified_id', $id_a)->get();
            // Carga los metas en las variables
            $this->setMeta();
            // Carga los datos de la web
            $for_meta_sis = $this->meta_sis;
            ...
            $this->user_sis = CoreUser::getUser();
            // Render del view
            return view('web.profile.user_classified.form', [
                'meta_lang' => $this->lang,
                'seo_title' => $this->seo_title,
                'seo_description' => $this->seo_description,
                'meta_sis' => $for_meta_sis,
    ...
    public function store(Request $request)
    {
        if ($request->get('process') === 'add') {
            ...
            $next_classified_id = $data_field->id;
            //check if image exist
            if ($request->hasFile('image')) {
                $images = $request->file('image');
                //Ruta para guardar
                $url_save_or = 'upload/classified/' . $next_classified_id . '/';
                ...
                foreach ($images as $key => $image) {
                    ...
                    $newPhoto->cupon_id = 0;
                    $newPhoto->event_id = 0;
                    $newPhoto->classified_id = $next_classified_id;
                    $newPhoto->news_id = 0;
                    $newPhoto->post_id = 0;
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($data_item != null) {
            if ($request->get('process') === 'edit') {
                ...
                //ID para guardar las fotos
                $next_classified_id = $data_item->id;
                //check if image exist
                if ($request->hasFile('image')) {
                    $images = $request->file('image');
                    //Ruta para guardar
